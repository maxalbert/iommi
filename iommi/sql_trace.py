...
def sql_debug_trace_sql(sql, params=None, **kwargs):
    ...
    if len(sql) > 10000:
        sql = '%s... [%d bytes sql]' % (
            re.sub(r'[\x00-\x08\x0b-\x1f\x80-\xff].*', '.', sql[:10000]),
            len(sql),
        )
    else:
...
def sql_debug_last_call(response):
        ...
        for count, _, logs in highscore[-number_of_offenders:]:
            if count > num_suspicious:  # 3 times is ok-ish, more is suspicious
                ...
                if hasattr(response, 'iommi_part'):
                    ...
                    sql_debug(
                        format_clickable_filename(filename, lineno, str(response.iommi_part._name)),
                        sql_trace=True,
                    )
    ...
    if settings.DEBUG:
        ...
        sql_debug(
            msg=(
                f'{request.META["REQUEST_METHOD"]} {request.get_full_path()} '
                f'-> {response.status_code} {duration}{total_sql_time}'
            )
        )
        sql_debug(
