...
def test_parse_errors(MyTestForm):
    ...
    assert_errors_and_matches_reg_exp(
        form.fields['joined']._errors,
        'Time data "foo" does not match any of the formats .*',
    )
    ...
    assert_errors_and_matches_reg_exp(
        form.fields['a_time']._errors,
        'Time data "bar" does not match any of the formats.*',
    )
    ...
    assert_errors_and_matches_reg_exp(
        form.fields['multi_choice_field']._errors,
        "[qw] not in available choices",
        count=2,
    )
...
def test_post_validation_and_error_checking_full():
    ...
    assert timeline == [
        ...
        },
        {
            ...
            'is_valid': False,
            'errors': {
                'fields': {'first': {'first error'}, 'second': {'second error'}},
                'global': {'global error'},
            },
        },
    ]
...
def test_hidden():
    soup = BeautifulSoup(
        Form(fields__foo=Field.hidden()).bind(request=req('get', foo='1')).__html__(),
        'html.parser',
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ['fisk', 'foo__hej', 'foo__hopp', 'bar__bink', 'bar__boink'],
            ['', 'bar', 'foo'],
        ),
    ],
)
...
def test_grouped_choices():
    ...
    assert f.fields.foo.grouped_choice_tuples == [
        ...
        (
            'b',
            [(3, '3', '3', False, 3), (4, '4', '4', False, 4), (5, '5', '5', False, 5)],
        ),
    ]
